...
filename="figure7_np_npr_(-)np_npr_10_whole_t_netw.txt"
filename="nt_ntr_tf_whole.txt"

savepath="D:/project/python/bioinfo/figures/nt/network/ntr"
...
dtnp = dtfm[dtfm['ntrtype'] == 'nt']
dtnpr = dtfm[dtfm['ntrtype'] == 'ntr']
...
ltname=['Antenna', 'Body', 'Fat_body','Haltere', 'Head', 'Heart', 'Leg','Oenocyte', 'Proboscis',
         'Testis','Trachea','Wing']
...
def drawGraph(ltname):
    ...
    for j in range(len(ltname)):
        ...
        dtnp = dtfm[(dtfm['ntrtype'] == 'nt') & (dtfm['tname'] == stname)]
        dtnpr = dtfm[(dtfm['ntrtype'] == 'ntr') & (dtfm['tname'] == stname)]
        ...
        legend_elements = [Line2D([0], [0], color='#FFDEA2', label='Edge'),
                           ...
                           Line2D([0], [0], marker='o', markersize=10, color='#8C8C8C',
                                  label='NT'),
                           Line2D([0], [0], marker='o', markersize=10, color='#FFBB00', label='NTR')]
        ...
        #nx.draw_networkx(G, node_color=colors, pos=nx.spring_layout(G, k=0.25, iterations=70), **options, ax=ax)
        #nx.draw_networkx(G, node_color=colors, pos=nx.spring_layout(G, k=0.15, iterations=50), **options, ax=ax)
        ...
        savepath = "D:/project/python/bioinfo/figures/nt/network/ntr"
        savefile = "figure7_tf_np_npr_network_" + ltname[j] + ".pdf"
        plt.savefig(os.path.join(savepath, savefile))


        savepath = "D:/project/python/bioinfo/figures/nt/network/ntr"
        savefile = "figure7_tf_np_npr_network_" + ltname[j] + ".png"
        plt.savefig(os.path.join(savepath, savefile))
