...
ltname=['Antenna', 'Body', 'Fat body','Haltere', 'Head', 'Heart', 'Leg','Oenocyte', 'Proboscis',
         'Testis','Trachea','Wing']
...
def drawGrophAll():
    ...
    plt.title("NT,NTR,TF", fontsize=20)
    ...
    nx.draw_networkx(G, node_color=colors, pos=nx.spring_layout(G, k=0.25, iterations=55), **options, ax=ax)
    ...
    savepath = "D:/project/python/bioinfo/figures/nt/network/ntr"
    savefile = "figure7_tf_nt_ntr_network_all.pdf"
    plt.savefig(os.path.join(savepath, savefile))

    savepath = "D:/project/python/bioinfo/figures/nt/network/ntr"
    savefile = "figure7_tf_np_npr_network_all.png"
    plt.savefig(os.path.join(savepath, savefile))
...
def drawGraphTissue(ltname):
    ...
    for j in range(17):
        ...
        savepath = "D:/project/python/bioinfo/figures/tf/network/group"
        savefile = "figure7_tf_npr_network_" + ltname[j] + ".pdf"
        plt.savefig(os.path.join(savepath, savefile))
