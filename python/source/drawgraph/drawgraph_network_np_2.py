...
savepath="D:/project/python/bioinfo/figures/tf/network/np"
...
filename="figure7_np+npr_np_npr_10_no_h_b_all_network_np.txt"   #np tissue 10
...
npuniqtfgenename= np.unique(nprtdata.motifname)
print("======agenename[0]=========",npuniqtfgenename[0])
for i in range(0,len(np.unique(nprtdata.motifname))):
...
def drawGraph(ltname):
    ...
    for j in range(17):
        ...
        # npgene sum(count)
        rgenecountsum = Body_df.groupby('npgenename')['npmotifregoccur'].sum()
        ...
        for index, row in Body_df.iterrows():
            ...
            G.add_node(row['npgenename'], nodesize=row['tfcount'])
        ...
        savepath = "D:/project/python/bioinfo/figures/tf/network/np"
        savefile = "figure7_tf_np_network_"+ ltname[j]+".pdf"
