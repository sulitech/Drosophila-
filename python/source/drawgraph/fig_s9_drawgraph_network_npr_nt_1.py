...
filename="npr_nt_group.txt"   #np tissue 10
savepath="D:/project/python/bioinfo/figures/nt/network/npr"
...
print(nprtdata.nprgenename.value_counts())
...
rgenenamedict ={}
npuniqrgenename= np.unique(nprtdata.nprgenename)
print("======agenename[0]=========",npuniqrgenename[0])
for i in range(0,len(np.unique(nprtdata.nprgenename))):
...
def drawGraph(ltname):
    ...
    for j in range(17):
        ...
        # npgene sum(count)
        rgenecountsum = Body_df.groupby('nprgenename')['nprexpress'].sum()
        ...
        for index, row in Body_df.iterrows():
            ...
            G.add_node(row['nprgenename'], nodesize=row['nprexpress'])
        ...
        for index, row in Body_df.iterrows():
            G.add_edges_from([(row['nt'], row['nprgenename'])])
        ...
        savepath = "D:/project/python/bioinfo/figures/nt/network/npr"
        #savefile = "figure5_nt_npr_network_" + ltname[j] + ".png"
        savefile = "figure5_nt_npr_network_" + ltname[j] + ".pdf"
