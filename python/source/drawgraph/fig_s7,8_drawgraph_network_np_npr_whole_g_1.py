...
if gender=="Female":
    #all
    filename = "figure7_np_npr_(-)np_npr_10_whole_t_f_netw.txt"
    #only female
    filename = "figure7_np_npr_(-)np_npr_10_whole_t_only_f_netw.txt"


else:
    #all
    filename="figure7_np_npr_(-)np_npr_10_whole_t_m_netw.txt"
    #only male
    filename = "figure7_np_npr_(-)np_npr_10_whole_t_only_m_netw.txt"
...
if gender=="Female":
    #all
    ltname = ['Testis', 'Antenna', 'Body wall', 'Fat body', 'Gut', 'Haltere', 'Heart', 'Leg',
              'Male reproductive glands', 'Malpighian tubule',
...
def drawGraph(ltname):
    ...
    for j in range(17):
        ...
        if(len(dtnp) >0):
            ...
            print("==gender==", gender, "==tissue==", ltname[j])
        ...
        plt.title(stname+"-"+gender , fontsize=20)
        ...
        savefile = "figure7_tf_np_npr_network_" + ltname[j] + "_"+gender+".pdf"
        plt.savefig(os.path.join(savepath, savefile))
        savefile = "figure7_tf_np_npr_network_" + ltname[j] + "_" + gender + ".png"
        plt.savefig(os.path.join(savepath, savefile))
        plt.show()
        print(ltname[j])
