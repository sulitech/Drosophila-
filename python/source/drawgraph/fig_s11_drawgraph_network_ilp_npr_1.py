...
filename="npr_10_regulon_tissue_ann_tfgene_reggene_list.txt"
...
filedir= os.path.join(filepath,filenamenpr)

print(filedir)

#pbarcodes = pd.DataFrame(os.path.join(chemopath,filename),index=0)
dtfm = pd.read_csv(os.path.join(filepath,filenamenpr), delimiter='\t')
...
ax.plot([0],[0],color='#72bbd0',label='NPR')
...
#nx.draw_networkx(G, node_color=colors,  pos=nx.spring_layout(G, k=0.25, iterations=10), **options,ax=ax)
#nx.draw_networkx(G, node_color=colors,  pos=nx.spring_layout(G, k=3, iterations=6), **options,ax=ax)
...
#Helvetica font 10~12
f = plt.figure(1)
#ax = f.add_subplot(1,1,1)  #눈금이 생긴다...
ax = plt.gca()
ax.plot([0],[0],color='#FFDEA2',label='Edge')
ax.plot([0],[0],color='#f09494',label='TFs')
ax.plot([0],[0],color='#72bbd0',label='NPR')
...
#nx.draw_networkx(G, node_color=colors, pos=nx.spring_layout(G, k=1.0, iterations=15), **options ,ax=ax)
#nx.draw_networkx(G, node_color=colors,  pos=nx.spring_layout(G, k=3, iterations=6), **options,ax=ax)

nx.draw_networkx(G, node_color=colors,  pos=nx.spring_layout(G, k=0.5 , iterations=20), **options , ax=ax , node_size=node_sizes)

#nx.draw_networkx(G, node_color=colors,  pos=nx.spring_layout(G, k=3.25, iterations=5), **options,ax=ax)
f.set_facecolor('w')
plt.legend(fontsize=12)
f.tight_layout()
